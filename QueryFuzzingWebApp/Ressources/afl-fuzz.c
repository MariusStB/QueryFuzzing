...
static u8* DTD(u64 cur_ms, u64 event_ms) {
    ...
    sprintf(tmp, "%s days, %u hrs, %u min, %u sec, %u ms", DI(t_d), t_h, t_m, t_s, t_ms);
    return tmp;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault, u8* crash_targets) {
    ...
    keep_as_crash:
        ...
        if (is_target) {
            unique_crashes++;
            fprintf(crashes_log, "%llu : %s --- %s --- [", unique_crashes, DTD(last_crash_time, start_time), DI(last_crash_execs));

            for (i = 0; i < targets_num; i++) {
                if (crash_targets && crash_targets[i]) {
                    fprintf(crashes_log, "%d ", i);
                }
            }
            fprintf(crashes_log, "]\n");
        }
...
static u8 save_if_crash(char** argv, void* mem, u32 len, u8 fault, u8* crash_targets) {
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
    ...
    u8 is_saved = save_if_interesting(argv, out_buf, len, fault, tmp_targets);
    ...
    if (target_path2) {
        ...
        if (is_target) {
            ...
            save_if_crash(argv, out_buf, len, fault, tmp_targets);
        }
    ...
    while ((sd_ent = readdir(sd))) {
        ...
        while ((qd_ent = readdir(qd))) {
            ...
            if (st.st_size && st.st_size <= MAX_FILE) {
                ...
                queued_imported += save_if_interesting(argv, mem, st.st_size, fault, empty_targets);
                syncing_party = 0;
